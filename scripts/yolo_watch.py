...
def _collect_buttons(element, out: list, depth: int = 0) -> None:
    """Walk the accessibility tree for buttons.

    Buttons are rarely direct children of the window: macOS permission dialogs
    nest them inside a group or sheet, so a one-level scan finds nothing at all.
    """
    if depth > 6:
        return
    for child in _ax_value(element, kAXChildrenAttribute) or []:
        ...
        if role == "AXButton":
            ...
            _collect_buttons(child, out, depth + 1)
...
def scan(rules: list[dict], seen: dict[str, float], streak: dict[str, int]) -> None:
    ...
    for info in infos:
        ...
        for rule in rules:
            ...
            # windows on the Mac. Before treating one as a permission dialog,
            # confirm it actually offers an Allow button. Without Accessibility
            # we cannot tell, so we stay quiet rather than guess -- the first
            # version of this rule alerted about Notes and FaceTime.
            if rule.get("require_allow_button") and not find_allow_button(pid, allowed):
                continue
